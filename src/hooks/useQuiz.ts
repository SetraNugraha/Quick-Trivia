...
export function useQuiz() {
  ...
  const [currQuiz, setCurrQuiz] = useState<number>(() =>
    JSON.parse(localStorage.getItem("currQuiz") || "0")
  );
  ...
  const [isFinishQuiz, setIsFinishQuiz] = useState<boolean>(
    localStorage.getItem("isFinishQuiz") === "true"
  );
  ...
  const handleAnswer = (answer: string) => {
    const correctAnswer: boolean =
      answer === questions[currQuiz].correct_answer;
